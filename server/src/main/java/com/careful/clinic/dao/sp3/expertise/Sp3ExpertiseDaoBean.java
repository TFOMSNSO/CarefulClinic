...
//		 							 "and  d.period between ' 201801 ' and ' 201802 ' "+  
	...
	public List<?> getResalt3a3b_expertise(String date1,String date2,String user, int firrstResult) {
		...
		String sb = "select p.FIO,p.dr, p.SMOID,p.SERPOLIS,p.NUMPOLIS,p.lpu,p.AMBKARTA, p.DAT_BEG,p.DAT_END,LPU_PRIK,p.s1,p.account,p.AC_DATE,p.REZOBR, p.ds1 as f_mkb_usl, f_person_telephone_v2@dome_dev(p.fam, p.im, p.ot, p.dr) as tel, p.id " 
		 +" from "
		// пункт 6 ( условия до union all)		
		 +" pat p left join gosp_eir_view@link_webofoms gosp on "
		    +" p.fam = upper(trim(gosp.fam)) and p.im = upper(trim(gosp.im)) and p.ot = upper(trim(gosp.ot)) and p.dr = to_date_gosp@link_webofoms(gosp.dr)" 
		    +" and p.dat_end <= to_date_gosp@link_webofoms(gosp.d_np) " 
		    +" and substr(p.ds1,0,1) = substr(gosp.mkb_priem,0,1) " 
		    +" and to_date_gosp@link_webofoms(gosp.d_np) >= '01.01.2018' , "
		 +" demand d "
		 +" where "
		 // по left join выбираем у кого нет госпитализации
		 +" gosp.fam is  null "
		 +" and "
		 +" d.id_demand = p.demand_id " 
		 +" and  p.caretype = 30 "  
		 ...
		 + g  
		 +" and  p.dat_end >= '01.01.2018' "  
		 //-- проверяем на отсутствие диспансерного наблюдения
		 +" and (p.nazr <> 5 and p.nazr <> 7 and p.nazr <> 2 and p.pr_d_n = 0) " 
		 // месы первого этапа в 2018 году
		 +" and  p.mes between '401048' and '401071' "
		 // если данные пролеченных были на втором этапе с месами 401079 401080 (то есть если на втором этапе была просто беседа то берем первый этап)
		 +" and (p.fio,p.dr)  in ( "
		                            +" select zz.fio,zz.dr "
		                            +" from "
		                            +" ( "
		                            ...
		                            +" where "
		                             ...
		                             +" and  p.dat_end >= '01.01.2018' "  
		                             +" and  p.mes between '401048' and '401080' "
		                             // берем только тех людей у которых есть первый этам по мэсам первого этапа текущего года (2018)
		                             +" and exists (select 1 from pat pp where pp.caretype = 30 and mes between '401048' and '401071'  and pp.fio = p.fio and pp.dr = p.dr) "
		                             +" ) zz "
		                             +" where zz.mes in (401079,401080) "
		...
		+" select  zz.FIO,zz.dr, zz.SMOID,zz.SERPOLIS,zz.NUMPOLIS,LPU,zz.AMBKARTA, zz.DAT_BEG,zz.DAT_END,zz.LPU_PRIK,zz.s1,zz.account,zz.AC_DATE,zz.REZOBR, zz.ds1 as f_mkb_usl, f_person_telephone_v2@dome_dev(zz.fam, zz.im, zz.ot, zz.drr) as tel, zz.id " 
		                          ...
		                          +" select p.mes,p.FIO,p.dr, p.SMOID,p.SERPOLIS,p.NUMPOLIS,p.lpu,p.AMBKARTA, p.DAT_BEG,p.DAT_END,LPU_PRIK,p.s1,p.account,p.AC_DATE,p.REZOBR, p.ds1, p.fam, p.im, p.ot, p.dr as drr, p.id " 
		                   		 ...
		                   		 // по left join выбираем у кого нет госпитализации
		                             ...
		                             +" and  p.dat_end >= '01.01.2018' "  
		                             +" and (p.nazr <> 5 and p.nazr <> 7 and p.nazr <> 2 and p.pr_d_n = 0) " 
		                             // диапазон месов первого, второго  этапов (на всякий случай)
		                             +" and  p.mes between '401048' and '401080' "
		                             // берем только тез людей у которых есть первый этам по мэсам первого этапа текущего года (2918)
		                             +" and exists (select 1 from pat pp where pp.caretype = 30  and mes between '401048' and '401071'  and pp.fio = p.fio and pp.dr = p.dr) "
		                             + "and p.id not in (select ot.id_pred from otkl_id ot where ot.id_pred = p.id) "
		                             +" ) zz "
		                             +" where zz.mes='401072' "
